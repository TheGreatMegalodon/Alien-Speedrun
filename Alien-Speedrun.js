...
const timing = {
	300: {
		txt1: `${format_time(300)}`,
		txt2: `Minutes on the clock`,
		color: `rgba(155, 255, 55, 0.8)`
	},
	600: {
		txt1: `${format_time(600)}`,
		txt2: `Minutes on the clock!`,
		color: `rgba(255, 155, 55, 0.8)`
	},
	1200: {
		txt1: `${format_time(1200)}`,
		txt2: `Minutes on the clock.!`,
		color: `rgba(255, 155, 55, 0.8)`
	},
	1800: {
		txt1: `${format_time(1800)}`,
		txt2: `Minutes on the clock..! Time is running high! too high..`,
...
var asteroid_positions = getCords(100, {
	cords: 100
}, false);
var asteroid_sizes = setVariables({
	order: 3,
	len: 10,
	gap: 2
});
var asteroid_velocity = getCords(6, {
	cords: 0
}, false).map(pos => pos / 12.5).filter(pos => pos !== 0);
		...
		switch (startingGame) {
			case true:
				if (time >= 0) {
					...
					if (instrutor_ended) game.ships.forEach(ship => {
						alert(ship, format_time(time + 1), `Starting...!`, "rgba(255,255,255, 0.8)", 1500);
					});
				} else {
					time = 0;
					startingGame = 1;
					game.addAlien({
						...
						crystal_drop: setVariables({
							order: stage + 4,
							len: 10,
							gap: 2
						})[~~(Math.random() * setVariables({
							order: stage + 4,
							len: 10,
							gap: 2
						}).length)],
					...
					game.ships.forEach(ship => {
						alert(ship, `First Alien!`, `Alien:  ${alienn.name}, difficulty:  ${alienn.difficulty[nextAlien_Level]}`);
					});
				}
				break;
			case 1:
			...
			case 2:
				if (time >= 0) {
					...
					endScoreboard(game);
					if (!instrutor_ended) game.ships.forEach(ship => {
						alert(ship, format_time(time + 1), `Ending...!`, "rgba(255,255,255, 0.8)", 1500);
					});
				} else {
					game.ships.forEach(ship => {
...
function paChangeMode(code, level) {
	return ((code == 16 && level == 3) || (code == 20 && level >= 1)) ? "active" : "passive";
}

function getNextAlien() {
	if (code === wavesInfo[stage].len) stage++, code = 1;
	else code++;
	return {
		code: wavesInfo[stage].codes[code].code,
		level: wavesInfo[stage].codes[code].level
	};
}

function isBoss(code, level) {
	return (code == 20 || code == 19 || code == 15 || code == 12 || (code == 16 && level == 3) || (code == 10 && level == 3));
}
	...
	switch (event.name) {
		case "alien_destroyed":
			...
			if (stage === 5) gameFinished(event.killer);
				...
				const crystals = isBoss(nextAlien_Code, nextAlien_Level) ? setVariables({
					order: (Math.pow(stage + 1, 3)) + 10,
					len: 10,
					gap: 2
				})[~~(Math.random() * setVariables({
					order: (Math.pow(stage + 1, 3)) + 10,
		...
		case "asteroid_destroyed":
			if (event.killer) {
				const point = event.asteroid.size > 30 ? setVariables({
					order: 3,
					len: 1,
					gap: 2
				}) : setVariables({
				...
				event.killer.set({
					score: event.killer.score + point[0]
				});
...
function alert(ship, Value1 = "", Value2 = "", Color = "rgba(255, 255, 255, 0.8)", time = 4000, height = {
	txt1: 18,
	txt2: 24
}) {
	...
	ship.setUIComponent({
		...
		components: [{
				type: "text",
				position: [0, height.txt1, 100, 5],
				color: Color,
				value: Value1
			},
			{
				type: "text",
				position: [0, height.txt2, 100, 4],
				color: Color,
				value: Value2
			}
		]
	...
	if (hide_after) {
		instructor_func = function() {
			...
			setTimeout(() => {
				ship.hideInstructor();
				if (allow) {
					if (!instrutor_ended) instrutor_ended = true, addObject("MapCenter", Objects.MapCenter, {
						x: 20 * 5,
						y: 20 * 5,
						z: -15,
						sx: 60,
	...
	let Scoreboard = {
		...
		components: [{
			...
			// txt
			{
				type: "text",
				position: [0, 60, 100, 8],
				value: dataInitValues[(dataInitIndex++) % dataInitValues.length],
				color: "rgba(255,255,255,0.8)",
				align: "center"
			}
		]
...
function updateScoreboard(game) {
	...
	let Scoreboard = {
		...
		components: [{
			...
			// Alien Name
			{
				...
				fill: `rgba(${colorAlien}, 0.4)`
			},
			{
				type: "box",
				position: [3, 65, 75 - 20, 10],
				fill: "rgba(155, 155, 155, 0.1)"
			},
			{
				type: "box",
				position: [79.5 - 20, 65, 19.5 + 20, 10],
				fill: "rgba(155, 155, 155, 0.2)"
			},
			{
				type: "text",
			...
			// Alien Difficulty
			{
				...
				fill: `rgba(${colorInformations}, 0.4)`
			},
			{
				type: "box",
				position: [3, 77, 75 - 15, 10],
				fill: "rgba(155, 155, 155, 0.1)"
			},
			{
				type: "box",
				position: [79.5 - 15, 77, 19.5 + 15, 10],
				fill: "rgba(155, 155, 155, 0.2)"
			},
			{
				type: "text",
			...
			},
			{
				...
				value: alienLB.difficulty[nextAlien_Level],
				color: `rgba(${colorInformations}, 0.8)`,
				align: "center"
			},
			// Asteroid Number
			{
	...
	game.ships.forEach(shiper => {
		if (timing.hasOwnProperty(time + 1)) alert(shiper, timing[time].txt1, timing[time].txt2, timing[time].color, 4000, {
			txt1: 75,
			txt2: 81
		});
...
addObject("lost_sector_aries", Objects.lost_sector_aries, {
	x: 20 * 5,
	y: 20 * 5,
	z: -60,
	sx: 2,
